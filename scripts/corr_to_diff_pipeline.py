...
la generación de matrices de diferencia de múltiples sujetos de manera
...
from tesis.features import diff_matrices_pipeline
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Pipeline de transformacion de matrices de correlación a matrices de diferencia"
    )
    parser.add_argument(
        "--corr-dir",
        type=Path,
        required=True,
        help="Directorio de origen (matrices de correlacion)",
    )
    parser.add_argument(
        "--scp-dir",
        type=Path,
        required=True,
        help="Directorio de origen (matrices scp)",
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
        required=True,
        help="Directorio de salida (matrices de diferencia)",
    )
    parser.add_argument(
        "--workers",
        type=int,
...
def main() -> None:
    ...
    lista_sujetos = sorted(p.name for p in args.corr_dir.iterdir() if p.is_dir())
    ...
    procesamiento_paralelo_por_sujeto(lista_sujetos= lista_sujetos,
                                      funcion_sujeto= diff_matrices_pipeline,
                                      workers= args.workers,
                                      paths= (args.corr_dir, args.scp_dir, args.output_dir),
                                      )
